...
export function ReserveScheduleManager({ categories, accounts, schedules, budgetTargets }: { categories: Item[]; accounts: Item[]; schedules: Schedule[]; budgetTargets: Record<string, number> }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      await save({ categoryId: categoryId as never, accountId: accountId as never, amount: Number(amountValue), dayOfMonth: Number(day), isActive: true });
  ...
  return <form onSubmit={submit} className="grid gap-3 rounded-xl border border-lime-200 bg-lime-50/60 p-4 sm:grid-cols-4">
    <p className="sm:col-span-4 text-sm text-slate-700">The amount starts from this category’s monthly budget target. On the due date, you can adjust and confirm it directly from the Dashboard.</p>
